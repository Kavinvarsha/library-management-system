...
// // Query a single integer value (first column of first row). Returns true if found.
...
//                 int id, avail;
//                 char title[255], author[200], isbn[50];
...
//             while (SQLFetch(stmt) == SQL_SUCCESS) {
//                 int id; char name[100], email[100], phone[20], type[20], status[12];
//                 SQLGetData(stmt, 1, SQL_C_SLONG, &id, 0, NULL);
...
// // helper: get config integer (MaxBooks or ReservationDuration)
...
// // check book availability (1 = available, 0 = not)
...
// // Issue a book to a member (validations)
// void issueBook() {
//     int memberID, bookID;
...
//         // if unavailable, check reservations: if the first pending reservation belongs to this member, allow
...
//                 return;
//             }
//             // else first reserver is the same member -> we will issue and mark reservation Completed below
//         } else {
...
//     // Insert transaction and update book availability in a simple transaction (no ODBC transactions used here)
...
//     // If there is a pending reservation for this member for this book, mark it Completed
...
// // Return book, calculate fine, and if reservation queue exists auto-issue to first reserver
// void returnBook() {
//     int transID;
//     cout << "Enter TransactionID: "; cin >> transID;
//     cin.ignore();

//     // validate transaction exists and not already returned
...
//     // Calculate fine and update ReturnDate and FineAmount
//     // FineRate from Config; doing calculation in SQL for simplicity (fallback to 5 if missing)
//     double fineRate = getFineRate(); // we will use this for message, but run update via SQL using value
//     string sqlUpdate = 
...
//     // Find BookID for this transaction
//     int bookID = 0;
...
//         // If there is a pending reservation, automatically issue to first pending member:
//         int nextMember = 0;
...
//                 // Create new transaction for that member and set book Availability back to 0 (auto-issue)
...
// // Reserve a book (create reservation queue entry)
// void reserveBook() {
//     int memberID, bookID;
//     cout << "Enter MemberID: "; cin >> memberID;
//     cout << "Enter BookID: "; cin >> bookID;
...
//     // Prevent duplicate pending reservation by same member for same book
...
// // View transaction history by member or book with pagination
...
//                 double fine;
//                 // Some columns may be NULL; fetch into buffers carefully
...
//         else if(choice=="6") { cout << "Bulk import currently not wired here.\n"; }
//     } while(choice!="7");
// }
...
//     // <-- update SERVER and DATABASE to match your environment if needed -->
...
// Check if record exists (returns true if any row)
bool exists(const string& sql) {
    SQLHSTMT stmt;
    if (!SQL_SUCCEEDED(SQLAllocHandle(SQL_HANDLE_STMT, dbc, &stmt))) return false;
    if (!SQL_SUCCEEDED(SQLExecDirect(stmt, (SQLCHAR*)sql.c_str(), SQL_NTS))) {
...
// Utility to export query results to CSV
        ...
        for (int i = 1; i <= cols; i++) {
            ...
            if (r == SQL_SUCCESS || r == SQL_SUCCESS_WITH_INFO) {
                if (ind == SQL_NULL_DATA) file << "";
                    ...
                    // Basic CSV escaping: wrap field in quotes if it contains comma or quote or newline
                    string val = buf;
...
// Top 10 issued books (display + auto-export)
...
// Most active members (display + auto-export)
...
// Fine collection summary (display + auto-export)
...
// Reports Menu (auto-export enabled)
void reportsMenu() {
    string choice;
    do {
        cout << "\n===== Reports Menu =====\n"
...
void booksMenu() {
    ...
    do {
        ...
        else if(choice=="6") { cout << "Bulk import currently not implemented here.\n"; }
    } while(choice!="7");
}
